...
/*   Updated: 2023/03/16 18:51:09 by gde-alme         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	_input_norm(int keycode, t_data *gdata, t_wdata *wdata)
{
	if (_can_walk(keycode, gdata, wdata) == 0)
	{
		...
		{
			gdata->player_pos_x += wdata->wp_dir.x * (float)WALKSIZE;
			gdata->player_pos_y += wdata->wp_dir.y * (float)WALKSIZE;
			_raycaster(gdata);
...
int	_input(int keycode, t_data *gdata)
{
	...
	if (_change_dir(keycode, gdata) > 0)
	{
		...
		return (_input_norm(keycode, gdata, &wdata));
}

int	_check_hit(t_data *gdata)
{
	if (worldMap[(int)gdata->p_pos.y][(int)gdata->p_pos.x] > 0)
		return (1);
	else
		return (0);
}
